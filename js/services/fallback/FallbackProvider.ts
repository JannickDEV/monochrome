...
export class FallbackProvider implements Provider {
    ...
    private async resolveProviderTrackId(targetProvider: Provider, id: string | number): Promise<string | number> {
        ...
        if (knownSourceProvider) {
            try {
                ...
                meta =
                    typeof knownSourceProvider.getTrackMetadata === 'function'
                        ? await knownSourceProvider.getTrackMetadata(id)
                        : null;

                if (!meta || !meta.title) {
                    meta =
                        typeof knownSourceProvider.getTrack === 'function'
                            ? await knownSourceProvider.getTrack(id)
                            : null;
                }
        ...
        if (isrc) {
            try {
                ...
                if (items.length === 0 && targetProvider.id === 'qobuz') {
                    console.warn(
                        `[Safeguard] ISRC search results on Qobuz turned up entirely empty for ${id}. Forcing fallback to TIDAL.`
                    );
                ...
                if (items.length > 0) {
                    ...
                    if (!match && items[0]) {
                        ...
                        if (!firstResult.isrc) {
                            try {
                                const trackMeta =
                                    typeof targetProvider.getTrackMetadata === 'function'
                                        ? await targetProvider.getTrackMetadata(firstResult.id)
                                        : typeof targetProvider.getTrack === 'function'
                                          ? await targetProvider.getTrack(firstResult.id)
                                          : null;
                                ...
                                if (trackMeta?.isrc?.toLowerCase() === isrc!.toLowerCase()) {
                                    ...
                                    console.warn(
                                        `[FallbackProvider] Rejecting fallback for ${id}: ISRC mismatch (expected ${isrc}, got ${trackMeta?.isrc})`
                                    );
                                }
                            } catch (err) {
                            ...
                            console.warn(
                                `[FallbackProvider] Rejecting fallback for ${id}: first result had ISRC ${firstResult.isrc} which didn't match ${isrc}`
                            );
                        }
                    }
                    ...
                    if (match) {
                        if (!meta && knownSourceProvider) {
                            try {
                                ...
                                console.warn(
                                    `[FallbackProvider] Could not fetch metadata for validation for ${id}:`,
                                    e
                                );
                            }
                        }
                        ...
                        if (meta && (meta.title || meta.name) && match.title) {
                            ...
                            const isTitleSimilar =
                                tTitle === mTitle || tTitle.includes(mTitle) || mTitle.includes(tTitle);

                            if (!isTitleSimilar) {
                                console.warn(
                                    `[FallbackProvider] ISRC match rejected due to completely different title! Expected "${meta.title}", got "${match.title}" (ISRC: ${isrc})`
                                );
                                match = null;
                            }
                        }
                ...
                try {
                    ...
                    const candidate = items.find((t: any) => {
                        const tTitle = String(t.title || '')
                            .toLowerCase()
                            .trim();
                        const tArtist = String(t.artist?.name || t.artists?.[0]?.name || '')
                            .toLowerCase()
                            .trim();
                        const titleOk =
                            tTitle === metaTitle || tTitle.includes(metaTitle) || metaTitle.includes(tTitle);
                        const artistOk =
                            !metaArtist ||
                            tArtist === metaArtist ||
                            tArtist.includes(metaArtist) ||
                            metaArtist.includes(tArtist);
                    ...
                    console.warn(
                        `[FallbackProvider] Title/artist search failed on ${targetProvider.name} for "${meta.title}":`,
                        err
                    );
                }
            }
        ...
        if (!match || !match.id) {
            throw new Error(
                `Cannot translate track ID ${id} to ${targetProvider.name} (no match found via ISRC or title/artist)`
            );
        }
        ...
        if (targetProvider.id === 'qobuz') {
            ...
            if (!resolvedStr.startsWith('q:') && /^\d+$/.test(resolvedStr)) {
                console.warn(
                    `[FallbackProvider] SAFETY NET: Resolved ID ${match.id} for Qobuz looks like a raw TIDAL ID. Rejecting to prevent wrong-provider playback.`
                );
                throw new Error(
                    `Resolved ID ${match.id} is a raw numeric ID — refusing to send to Qobuz (likely a TIDAL ID)`
                );
            }
        }
    ...
    private async executeWithFallback<T>(
        ...
        for (const provider of providers) {
            try {
                ...
                errors.push(err);
                console.warn(
                    `[FallbackProvider] ${provider.name} failed for ${operation}(${JSON.stringify(args)}): ${err.message || err}. Falling back to next provider.`
                );
            }
        }
    ...
    async search(query: string, options: SearchOptions = {}): Promise<SearchResults> {
        return this.executeWithFallback(
            ...
            (res) =>
                !res ||
                (!res.tracks?.items?.length &&
                    !res.albums?.items?.length &&
                    !res.artists?.items?.length &&
                    !res.playlists?.items?.length)
        );
    }
        ...
        return this.executeWithFallback(
            ...
            async (p) => {
                ...
                if (p.id === 'qobuz' && /^\d+$/.test(String(targetId))) {
                    throw new Error(
                        `[getStreamUrl] Refusing to stream raw numeric ID ${targetId} on Qobuz — likely a TIDAL ID`
                    );
                }
    ...
    async getTrackForDownload(id: string | number, quality?: string): Promise<any> {
        return this.executeWithFallback(
            ...
            async (p) => {
                ...
                if (p.id === 'qobuz' && /^\d+$/.test(String(targetId))) {
                    throw new Error(
                        `[getTrackForDownload] Refusing to download raw numeric ID ${targetId} on Qobuz — likely a TIDAL ID`
                    );
                }
